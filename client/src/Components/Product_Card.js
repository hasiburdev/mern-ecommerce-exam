...
// compare modal button fuction
    const compare= async(id)=>{
      setmodal3Visible(true)
        try {
            const compareproducts= await axios.get(`http://localhost:8000/api/all_products${id}`)
...
// ........................
         ...
         <div className='product-name-price' >
             <h3 className='product-name' onClick={()=>details(products._id)}>{products.pname}</h3>
             <h4 className='product-price'>{`$.${products.price}`}</h4>
         </div>  
        
         {/* {overlayid == products._id ?  */}
               {/* <div className='overlay'> */}
                     {cstate.cartItems.map((item)=> item._id == products._id 
                       ?      
                             item.quantity == 0  ?  
                             <div style={{color:"white", fontSize:"24px",display:"flex",justifyContent:"center",alignItems:"center",height:"100%"}} onClick={()=>addcart(products._id)}> Add Product </div>
                             :
                                 <div className='inde'>
           ...
           <h3 className='add-cart-btn' onClick={()=>addcart(products._id)}> Add Cart </h3>
                    ...
                    <Select mode="tags" style={{ width: '100%' }} placeholder="Tags Mode" onChange={coparesearch1}>
                     { catProducts.map(items=>(
                          <Option key={items._id} >
                             <p onClick={()=>coparesearchproduct1(items._id)}>{items.pname}</p>
                          </Option>
                     ))}
                     </Select>
                              ...
                              <div className='search-list' >
                                  <ul> 
                                    {compareSearch2.map(items=>(
                                      <li onClick={()=>copareSearchProduct2(items._id)}>{items.pname}</li>
                                    ))}
                                  </ul>
                              </div>
                         :
        ...
        <Drawer title="Shooping Cart" placement="right" onClose={onClose} visible={visible}>
                   <div className='side-cart'>
                     {cstate.cartItems.length  ? cstate.cartItems.map(items=>(             
                             items.quantity <1 ? dispatch({type :"REMOVE_CART", payload: items._id}) :   
