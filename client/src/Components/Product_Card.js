...
const Product_Card = (props) => {
  ...
  const compare = async (id) => {
    ...
    try {
      const compareproducts = await axios.get(
        `${backendUrl}/api/all_products${id}`
      );
  ...
  const details = async (id) => {
    ...
    try {
      const products = await axios.get(`${backendUrl}/api/all_products/${id}`);
      dispatch({
        type: "FATCH_SUC",
  ...
  const addcart = async (id) => {
    try {
      ...
      setoverlayid(newproduct._id);
      setVisible(true);
      let exproduct = cstate.cartItems.find(
  ...
  const quantity = (item, quantity) => {
    cdispatch({ type: "ADD_CART", payload: { ...item, quantity } });
  };
  const initvalue = {
    singleproducts: {},
    loading: true,
  ...
  const copareSearch2 = (e) => {
    ...
    let match = catProducts.map((items) =>
      items.pname.includes(e.target.value) ? items : ""
    );
  ...
  return (
    <>
        ...
        catProducts.map((products) => (
          ...
          <div className="p-card-body">
            <div className="love-compare">
              ...
              >
                <SyncOutlined onClick={() => compare(products._id)} />
              </Tooltip>
              {props.sold ? (
                <Tag color="#434E6E">Sold : {products.sold}</Tag>
              ) : (
                ""
              )}
            </div>
            ...
            <h3 className="add-cart-btn" onClick={() => addcart(products._id)}>
              {" "}
              Add Cart{" "}
            </h3>
      ...
      <Modal
        ...
        onCancel={() => dispatch({ type: "REMOVE" })}
      >
        <div className="image">
          <img src={img} />
        </div>
        <div className="single-details">
          <h1 className="details-pname">
            {pname}
            <Badge.Ribbon
              text={stock <= 1 ? "Out of Stock" : "In Stock"}
              color={stock <= 1 ? "red" : "cyan"}
            ></Badge.Ribbon>
          </h1>
          ...
          <div>
            ...
            <div className="usecupon">
              ...
              <Button onClick={() => applycupon(discout, price, cupon)}>
                Apply
              </Button>
            </div>
          </div>
          ...
          <Card title="Copare with another product">
            ...
            <Card.Grid style={gridStyle} hoverable={false}>
              <Select
                ...
                onChange={coparesearch1}
              >
                {catProducts.map((items) => (
                  <Option key={items._id}>
                    <p onClick={() => coparesearchproduct1(items._id)}>
                      {items.pname}
                    </p>
                  </Option>
                ))}
                ...
                <div className="search-list">
                  <ul>
                    {compareSearch2.map((items) => (
                      <li onClick={() => copareSearchProduct2(items._id)}>
                        {items.pname}
                      </li>
                    ))}
                  </ul>
      ...
      >
        <div className="side-cart">
          {cstate.cartItems.length ? (
            cstate.cartItems.map((items) =>
              items.quantity < 1 ? (
                dispatch({ type: "REMOVE_CART", payload: items._id })
              ) : (
                <div className="single-card">
                  <img src={items.img} />
                  <div className="name-price">
                    <h3>{items.pname}</h3>
                    <h4>Price : ${items.price * items.quantity}</h4>
                  </div>
                  <div className="sbutton">
                    <Button.Group>
                      <Button
                        disabled={items.quantity < 1 ? true : false}
                        onClick={() => quantity(items, items.quantity - 1)}
                        icon={<MinusOutlined />}
                      />
        ...
        <div className="total-btn">
          ...
          {!cstate.cartItems.length ? (
            <Button disabled>
              <Link to="/Continue_Order"> Continue Order </Link>
            </Button>
          ) : (
            <Button>
              <Link to="/Continue_Order"> Continue_Order </Link>
            </Button>
          )}
        </div>
      </Drawer>
